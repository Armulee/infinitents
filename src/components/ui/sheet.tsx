...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-popover p-6 shadow-2xl transition ease-in-out data-[state=closed]:duration-200 data-[state=open]:duration-300 data-[state=open]:animate-in data-[state=closed]:animate-out",
  {
    variants: {
      side: {
  ...
  return (
    <SheetPortal>
      ...
      <SheetPrimitive.Content className={cn(sheetVariants({ side }), className)} {...props}>
        {children}
        <SheetPrimitive.Close className="absolute right-4 top-4 rounded-md p-1 text-muted-foreground opacity-70 transition-opacity hover:bg-secondary hover:opacity-100 focus:outline-none">
          <X className="size-4" />
...
function SheetDescription({
  ...
  return (
    <SheetPrimitive.Description className={cn("text-[13.5px] text-muted-foreground", className)} {...props} />
  );
}
